...
def create_dummy_job(db, process="terminaciones", is_test=0, priority=3):
    ...
    with db.connect() as con:
        con.execute(
            ...
            ) VALUES (?, ?, 'P1', '10', 'M1', 1, ?, ?, 'pending')
            """,
            (job_id, process, priority, is_test)
        )
    return job_id
...
def test_config_change_recalculates_priorities(temp_db):
    ...
    job_normal = create_dummy_job(db, priority=3)
    job_urgent = create_dummy_job(db, priority=2)
    job_test = create_dummy_job(db, priority=1, is_test=1)
    ...
    assert p_normal == 4, "Normal job should update 3 -> 4"
    assert p_urgent == 3, "Urgent job should update 2 -> 3"
